...
import kyash from "../assets/kyash.jpg";

<div class="profile-intro">
  <img class="profile-icon" src={icon.src} alt="いらにかのプロフィールアイコン" />
  <div class="profile-intro-text">
    <p>名前：いらにか（iranika）</p>
    <p>※Twitterのアイコンは頻繁に変わります。</p>
  </div>
</div>
...
雑なおもちゃ箱→[ブックマークレット ボックス](/bookmarklet/)
...
const Language = [
  PowerShell, Javascript/Typescript, Rust, Python,
  Nim, PHP, HTML/CSS,
]
const WebFramework = [Vue, Astro]
const OS = [Ubuntu, CentOS, WindowsServer, Windows]
const CloudService = [
  GCP, AWS, ConoHa, Cloudflare, Firebase, NewRelic,
]
const DevelopApps = [
  ShellScript, SPA, WebExtension, Electron(desktop, mobile),
]
```
...
1. Kyash
    ...
    <img class="donation-qr" src={kyash.src} alt="Kyash の送金用 QR コード" />
